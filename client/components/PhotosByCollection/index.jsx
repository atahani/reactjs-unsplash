...
class PhotosByCollection extends Component {
  ...
  render() {
    const {loggedInUsername, collection, photos, nextPhotosLink, onGetCollectionPhotos} = this.props;
      ...
      if (collection) {
        return (
          <Header>
            <Title>
              {collection.title}
              {loggedInUsername === collection.user.username
...
const mapStateToProps = (state, props) => {
  ...
  return {collection, loggedInUsername: state.user.user_profile.username, photos: state.items.photos, nextPhotosLink: state.items.photos_attr.next};
};
