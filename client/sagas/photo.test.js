...
describe('Photos Sagas Flow', () => {
  ...
  describe('Like Photo Flow', () => {
    ...
    it('pass likePhoto action then run jobStatus action', () => {
      expect(data.gen.next(likePhoto('123')).value).toEqual(put(jobStatus(true)));
    });
    ...
    it('should fork error when error happend in API request', () => {
      data
        .gen
        .next(likePhoto('123')); // run jobStatus(true)
      data
        .gen
        .next(); // call the likeAPI
      const fakeAPI = () => ({
        response: false,
        error: {
          code: 404
      ...
      data
        .gen
        .next(fakeAPI());
      expect(data.gen.next().value).toEqual(fork(handleCommonErr, {
        code: 404
      }, LIKE_PHOTO, {id: "123"}));
    });
  });
  ...
  describe('Un Like Photo Flow', () => {
    ...
    it('pass unLikePhoto action then run jobStatus(true)', () => {
      expect(data.gen.next(unLikePhoto('123')).value).toEqual(put(jobStatus(true)));
    });
  ...
  describe('search in photos flow', () => {
    ...
    it('run action and then jobsStats(true)', () => {
      expect(data.gen.next(searchInPhotos('/search&q=iran')).value).toEqual(put(jobStatus(true)));
    });
  ...
  describe('get photo flow', () => {
    ...
    it('run getPhoto action then jobStatus(true) should run', () => {
      expect(data.gen.next(getPhoto('123')).value).toEqual(put(jobStatus(true)));
    });
    ...
    it('setItem() action should run', () => {
      expect(data.gen.next().value).toEqual(put(setItem('photos', fakeAPI.response)));
    });
  });
